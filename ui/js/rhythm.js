...
function setupRhythmGame(config) {
    ...
    const backupKeys = ['A', 'S', 'D', 'J', 'K', 'L', 'Q', 'W', 'E', 'R', 'T', 'Y'];
    while (rhythmConfig.keys.length < rhythmConfig.lanes) {
        rhythmConfig.keys.push(backupKeys[rhythmConfig.keys.length] || String.fromCharCode(65 + rhythmConfig.keys.length));
    }
...
function handleRhythmKeyPress(e) {
    ...
    const keyPressed = String.fromCharCode(e.keyCode).toUpperCase();
...
function handleRhythmKeyRelease(e) {
    ...
    const keyReleased = String.fromCharCode(e.keyCode).toUpperCase();
...
function resetRhythmGame() {
    ...
    $('#rhythm-message').text(`Missed 0/${maxMissedNotes} notes allowed`);
}
